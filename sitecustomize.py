"""J.A.R.V.I.S. NEO runtime compatibility, voice fallback and UI bridge."""
...
def _install_core_runtime_guards(assistant) -> None:
    """Fix the actual packaged assistant runtime, not only the newer HUD path."""
    ...
    if speech_cls is None or getattr(speech_cls, "_neo_voice_fallback", False):
        return
    ...
    async def safe_say(self, text):
        ...
        if state is None or not state.voice_enabled or not text:
            return
        path = assistant.BASE_DIR / f"speech_{assistant.time.time_ns()}.mp3"
        state.is_speaking = True
        assistant.signals.speaking_change.emit(True)
        spoken = False
        try:
            if assistant.edge_tts is not None and assistant.PYGAME_OK:
                communicate = assistant.edge_tts.Communicate(
                    text,
                    assistant.VOICE,
                    rate=assistant.CONFIG.get("tts_rate", "+5%"),
                    volume=assistant.CONFIG.get("tts_volume", "+0%"),
                )
                await communicate.save(str(path))
                if not assistant.pygame.mixer.get_init():
                    assistant.pygame.mixer.init(frequency=44100, size=-16, channels=2, buffer=1024)
            ...
            assistant.log.warning(f"Edge TTS indisponible, fallback vocal local : {exc}")
        finally:
            try:
                if path.exists():
                    path.unlink()
        ...
        if not spoken and sys.platform == "win32":
            ...
            try:
                ...
                assistant.log.error(f"Fallback vocal Windows échoué : {exc}")

        if not spoken:
            assistant.log.error("Aucun moteur vocal disponible pour J.A.R.V.I.S.")
        state.is_speaking = False
        assistant.signals.speaking_change.emit(False)

    speech_cls._say = safe_say
    speech_cls._neo_voice_fallback = True


def _install_mini_hud(assistant) -> bool:
    """Attach the floating mini HUD to the real packaged JarvisWindow."""
    window_cls = getattr(assistant, "JarvisWindow", None)
    if window_cls is None or getattr(window_cls, "_neo_mini_hud_wired", False):
        return False
    try:
    ...
    def init_with_mini(self, *args, **kwargs):
        original_init(self, *args, **kwargs)
        try:
            self._neo_mini_hud = MiniHud()
            self._neo_mini_hud.place_top_right()
            self._neo_mini_hud.show()

            def set_state(value):
                state = str(value).upper()
                self._neo_mini_hud.set_state(state)

            def set_stats(data):
                self._neo_mini_hud.set_stats(
                    f"{int(data.get('cpu', 0))}%",
                    f"{int(data.get('ram', 0))}%",
                )

            def sync_devices():
                self._neo_mini_hud.set_devices(
                    bool(assistant.CONFIG.get("microphone_enabled", True)),
                    bool(getattr(assistant.state, "voice_enabled", True)),
                )

            assistant.signals.status_change.connect(set_state)
            assistant.signals.listening_change.connect(lambda active: set_state("LISTENING" if active else "ONLINE"))
            assistant.signals.speaking_change.connect(lambda active: set_state("SPEAKING" if active else "ONLINE"))
            assistant.signals.stats_update.connect(set_stats)
            assistant.signals.model_tier_change.connect(lambda tier: self._neo_mini_hud.set_ai(str(tier)))
            sync_devices()
            self.destroyed.connect(self._neo_mini_hud.close)
        except Exception as exc:
            try:
                assistant.log.warning(f"Mini HUD non chargé : {exc}")
            except Exception:
                pass

    window_cls.__init__ = init_with_mini
    window_cls._neo_mini_hud_wired = True
    return True


def install_runtime_fixes(assistant) -> None:
    """Public bootstrap used by the packaged launcher before assistant.main()."""
    _install_core_runtime_guards(assistant)
    _install_mini_hud(assistant)
...
def _bootstrap_runtime() -> None:
    ...
    for _ in range(240):
        ...
        if assistant is not None:
            install_runtime_fixes(assistant)
