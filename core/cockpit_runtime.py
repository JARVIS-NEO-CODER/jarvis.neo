...
class CockpitRuntime:
    ...
    @property
    def hud(self): return getattr(self.assistant, "cockpit", None)
    def show(self, panel_id, title, content="", kind="info", source=""):
        hud = self.hud
        if hud is None: return False
        try: hud.enqueue_dynamic_panel(panel_id, title, content, kind, source); return True
        except Exception: return False
    def remove(self, panel_id):
        hud = self.hud
        if hud is None: return False
        try: hud.enqueue_remove_dynamic_panel(panel_id); return True
        except Exception: return False
    def clear(self):
        hud = self.hud
        if hud is None: return False
        try: hud.enqueue_clear_dynamic_panels(); return True
        except Exception: return False
    ...
    def _show_search_results(self, query, image=False):
        ...
        try: results = self.search.search(query, limit=2)
    ...
    def handle_directive(self, text):
        raw = str(text).strip(); low = raw.lower()
        if not raw: return False
        if re.search(r"(?:efface|vide|supprime)\s+(?:tous\s+)?(?:les\s+)?panneaux", low): return self.clear()
        match = re.search(r"(?:ferme|supprime)\s+(?:le\s+)?panneau\s+([\w.-]+)", low)
        if match: return self.remove(match.group(1))
        match = re.search(r"(?:affiche|montre)\s+(?:une\s+)?(?:recherche|résultats?)\s+(?:web|internet)\s+(?:sur|pour)\s+(.+)", raw, re.I)
        if match: return self._show_search_results(match.group(1))
        match = re.search(r"(?:affiche|montre)\s+(?:des\s+)?images?\s+(?:sur|de|pour)\s+(.+)", raw, re.I)
        if match: return self._show_search_results(match.group(1), image=True)
    ...
    def enrich_common_request(self, text):
        low = str(text).lower(); shown = False
        ...
        if not weather and re.search(r"\b(?:cherche|recherche|trouve)\b", low) and "web" not in low:
            ...
            if query and len(query) <= 180: shown = self._show_search_results(query) or shown
        return shown
    ...
    def _parse_ai_markers(self, response):
        ...
        for match in list(_MARKER.finditer(response)):
            ...
            if kind == "web_search": self._show_search_results(spec.get("query", ""), image=False)
            elif kind == "image_search": self._show_search_results(spec.get("query", ""), image=True)
            elif kind in {"web", "image"} and self._url(source): self.show(panel_id, title, content, kind, source)
            elif kind in {"info", "status", "notification"}: self.show(panel_id, title, content, kind, source)
    ...
    def install(self):
        ...
        if processor is not None:
            self._original_process = processor.process; runtime = self
            @wraps(self._original_process)
            def wrapped_process(text):
                if runtime.handle_directive(text): return "Affichage dynamique mis à jour dans le cockpit."
                result = runtime._original_process(text); runtime.enrich_common_request(text); return result
            processor.process = wrapped_process
        self._patch_chat(ConversationAI); self._installed = True

    def _patch_chat(self, conversation_cls):
        if getattr(conversation_cls, "_neo_cockpit_patched", False): return
        original = conversation_cls.chat; runtime = self
        @wraps(original)
        def wrapped_chat(self_ai, messages, *args, **kwargs):
            ...
            if items and items[0].get("role") == "system":
                first = dict(items[0]); first["content"] = first.get("content", "") + protocol; items[0] = first
            result = original(self_ai, items, *args, **kwargs)
            return runtime._parse_ai_markers(result)
        conversation_cls.chat = wrapped_chat; conversation_cls._neo_cockpit_patched = True
