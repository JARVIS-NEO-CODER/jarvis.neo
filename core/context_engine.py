...
class ContextEngine:
    ...
    def detach_from_bus(self, event_bus: Any | None = None) -> None:
        bus = event_bus or self.bus
        if bus is None or not self._attached:
            return
        bus.unsubscribe("app.started", self._on_app_started)
        bus.unsubscribe("system.metric", self._on_system_metric)
        self._attached = False
    ...
    def _on_app_started(self, event: Event) -> None:
        ...
        if context:
            self._set_context(context, trigger=str(payload.get("app", event.name)), confidence=1.0)
        elif activity:
            self._set_context(activity, trigger=str(payload.get("app", event.name)), confidence=1.0)
    ...
    def _on_system_metric(self, event: Event) -> None:
        ...
        if active_process or active_window:
            self._set_context("ACTIVE", trigger=active_process or active_window, confidence=0.55)
        elif cpu is not None and cpu >= 90:
            self._set_context("RESOURCE_HEAVY", trigger="system.metric", confidence=0.65)

    def _set_context(self, context: str, *, trigger: str, confidence: float) -> None:
    ...
    def learn_current_context(self, result: ContextResult) -> None:
        if result.name == "UNKNOWN" or result.confidence < 0.5:
            return
        self.memory.remember_fact("context", f"last_{result.name}", result.name, confidence=result.confidence, source="context_engine")
