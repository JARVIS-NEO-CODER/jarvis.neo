...
def _show_cockpit(assistant: Any) -> None:
    def render() -> None:
...
def _show_agent_event(assistant: Any, state: str, task: dict[str, Any]) -> None:
    def render() -> None:
        ...
        if cockpit is None:
            return
        try:
            if not cockpit.isVisible(): cockpit.show()
            ...
            if state == "completed":
                ...
                try:
                    ...
                    if isinstance(payload, dict):
                        ...
                        if kind == "image_search": result = f"Recherche d'images terminée : {payload.get('query') or goal}"
                        elif payload.get("kind"): result = f"Résultat {payload.get('kind')} : {payload.get('query') or payload.get('message') or 'opération terminée'}"
                except Exception: pass
                cockpit.show_dynamic_panel("agent-result", "MISSION TERMINÉE", result, "success", "neo-agent")
            elif state == "failed": cockpit.show_dynamic_panel("agent-result", "MISSION ÉCHOUÉE", str(context.get("last_error") or "La mission autonome a échoué."), "error", "neo-agent")
            elif state == "waiting_approval": cockpit.show_dynamic_panel("agent-approval", "AUTORISATION REQUISE", str(context.get("waiting_message") or "Une action nécessite une autorisation avant de poursuivre."), "warning", "neo-agent")
            else: cockpit.show_dynamic_panel("agent-progress", "MISSION AUTONOME", goal, "info", "neo-agent")
        except Exception: pass
    _cockpit_dispatch(assistant, render)
...
def _show_image_results(assistant: Any, query: str) -> None:
    def worker() -> None:
        ...
        def render() -> None:
            cockpit = _get_cockpit(assistant)
            if cockpit is None: return
            cockpit.show(); cockpit.raise_(); cockpit.activateWindow()
            if not results:
                cockpit.show_dynamic_panel("image-search", "RECHERCHE D'IMAGES", f"Aucun résultat pour « {query} ».", "warning", "neo-agent"); return
            for index, item in enumerate(results[:8]): cockpit.show_dynamic_panel(f"image-search-{index}", item.title or query, "", "image", item.url)
        _cockpit_dispatch(assistant, render)
...
def _bind_active_window(runtime, components) -> None:
    ...
    if window_cls is None or getattr(window_cls, "_neo_agent_window_bound", False): return
...
def install() -> None:
    ...
    if getattr(original, "_neo_agent_bridge", False): _INSTALLED = True; return
    ...
    def on_event(event: str, payload: dict[str, Any]) -> None:
        task = payload.get("task", {}); state = task.get("state"); assistant = getattr(components, "_neo_assistant", None)
        if state: _show_agent_event(assistant, state, task)
    cfg = components._cfg()
    try: permission_mode = max(1, min(3, int(cfg.get("agent_permission_mode", 3))))
    except (TypeError, ValueError): permission_mode = 3
    try: max_steps = max(1, min(500, int(cfg.get("agent_max_steps", 80))))
    except (TypeError, ValueError): max_steps = 80
    ...
    try: _RUNTIME.recover()
    except Exception: pass
    def ask_ai(self, text: str):
        goal = str(text).strip()
        if not goal: return "Directive vide."
        try:
        ...
        except Exception: return original(self, text)
    def image_search(self, query: str):
        query = str(query).strip()
        if not query: return "Sujet de recherche d'images manquant."
        assistant = getattr(self, "_neo_assistant", None) or getattr(components, "_neo_assistant", None) or getattr(runtime, "_neo_assistant", None)
        if assistant is not None:
            _show_image_results(assistant, query); return f"Recherche d'images lancée pour « {query} »."
    ...
    if original_image_search is not None: components.CommandProcessor.image_search = image_search
    _INSTALLED = True
